...
const Navbar = () => {
  ...
  return (
    <div name='home' className='fixed flex w-full h-fit justify-between items-center px-5 md:px-20 sm:py-5 bg-[#020b1a] z-10 '>
       <a href=""> <img className='w-[140px] sm:w-[190px]' src={logo} alt="logoImage" /> </a>

        <ul className=' hidden md:flex w-fit gap-6 text-gray-50 font-semibold'>
        ...
        <div onClick={handleClick} className='md:hidden z-10 cursor-pointer text-2xl text-gray-300'>
         {!nav ? <FaBars /> : <FaTimes />}
         </div>

           {/* MOBILE MENU  */}
            <ul className={!nav ? 'hidden' : 'absolute top-0 left-0 w-full h-screen bg-[#020b1a] text-gray-300 flex flex-col justify-center items-center'}>
